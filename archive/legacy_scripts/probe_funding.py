...
import os, re, math, json, time, argparse, csv
...
TRUSTED_SOURCES = {
    ...
    "globenewswire.com", "news.yahoo.com", "venturebeat.com",
    "crunchbase.com", "crunchbase.com", "medium.com", "blog.google",
    "substack.com", "pitchbook.com", "tech.eu", "sifted.eu"
}

# Verbs/phrases that usually indicate a funding announcement
FUNDING_VERBS = [
    "raises", "raised", "announces funding", "announced funding",
    "closes", "closed", "secur(es|ed)", "lands", "snags", "bags",
    "series", "seed round", "pre-seed", "angel round", "venture round",
    "financing", "round"
]
...
def google_cse_search(query: str, api_key: str, cse_id: str, num: int = 10, date_restrict: Optional[str] = None) -> List[Dict[str, Any]]:
    """
...
def score_candidate(item: Dict[str, Any], company_domain: Optional[str], lookback_days: int) -> float:
    """
    Heuristic scoring:
    ...
    if contains_funding_verb(title) or contains_funding_verb(snippet) or contains_funding_verb(text):
        score += 0.20
...
def crunchbase_enrich(name: Optional[str], domain_root: Optional[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        for body in payloads:
            r = requests.post(f"{BASE}/searches/organizations", headers=H, json=body, timeout=DEFAULT_TIMEOUT)
        ...
        r = requests.post(f"{BASE}/entities/organizations/{org_id}", headers=H, json=body, timeout=DEFAULT_TIMEOUT)
        if r.status_code != 200:
            return {}
...
def build_queries(name: str,
                  domain: Optional[str],
                  owners: Optional[List[str]] = None,
                  aka: Optional[List[str]] = None) -> List[str]:
    qs = []
    ...
    funding_terms = [
        "raises", "raised", "fundraise", "funding", '"funding round"', "seed round",
        '"pre-seed"', '"Series A"', '"Series B"', '"Series C"', "venture round",
        "financing", '"led by"', '"co-led by"', '"participation from"'
    ]
...
def write_csv(path: str, candidates: List[Dict[str, Any]]) -> None:
    fields = [
        "score","source","url","title","published_at",
        "amount_usd","round_type","announced_on","investors","snippet"
    ]
...
def pretty_print(result: Dict[str, Any]) -> None:
    ...
    print(f"Owners: {', '.join(q.get('owners') or []) or '—'} | AKA: {', '.join(q.get('aka') or []) or '—'}")
    print(f"Lookback: {q.get('lookback_days')} days | Candidates returned: {len(result.get('candidates') or [])}")
    ...
    if best:
        ...
        print(f"  Source : {best.get('source')}  |  Date: {best.get('published_at') or facts.get('announced_on') or '—'}")
    ...
    if result.get("crunchbase_hint"):
        ...
        print("\nCrunchbase (hint):")
        if cb.get("last_round_type") or cb.get("last_round_amount_usd") or cb.get("last_round_date"):
            print("  Last round:", cb.get("last_round_type") or "?", "|",
...
def main():
    ...
    p.add_argument("--fetch-pages", type=lambda s: str(s).lower() in ("1","true","yes","y"), default=True)
