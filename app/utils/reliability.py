...
from app.core.exceptions import (
    CircuitBreakerError, RateLimitError, TimeoutError as SeeRMTimeoutError,
...
class AdaptiveRateLimiter:
    ...
    def _refill_tokens(self):
        ...
        self.tokens = min(
            self.burst_size,
            self.tokens + elapsed * self.current_calls_per_second
        )
    ...
    def on_success(self):
        ...
        if self.consecutive_successes >= 10:
            self.current_calls_per_second = min(
                self.base_calls_per_second * 1.5,
                self.current_calls_per_second * 1.1
            )
            self.consecutive_successes = 0
    ...
    def on_error(self):
        ...
        if self.consecutive_errors >= 3:
            self.current_calls_per_second = max(
                self.base_calls_per_second * 0.5,
                self.current_calls_per_second * 0.8
            )
            self.consecutive_errors = 0
...
class ParallelProcessor:
    ...
    def process_batch(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all tasks
            future_to_item = {
                executor.submit(safe_process_item, item): item
                for item in items
            }
...
def get_circuit_breaker_status() -> Dict[str, Any]:
    ...
    if hasattr(with_circuit_breaker, '_breakers'):
        return {
            name: breaker.status
            for name, breaker in with_circuit_breaker._breakers.items()
        }
    return {}
