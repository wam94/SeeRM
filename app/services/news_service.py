...
from app.core.exceptions import WorkflowError, ExternalServiceError, ValidationError
from app.core.models import (
    Company, NewsItem, CompanyIntelligence, ProcessingResult, 
    WorkflowType, ProcessingStatus, NewsItemSource
)
from app.data.gmail_client import EnhancedGmailClient
from app.data.notion_client import EnhancedNotionClient
from app.utils.reliability import with_circuit_breaker, with_retry, ParallelProcessor
...
class NewsCollector:
        ...
        if company.beneficial_owners:
            for owner in company.beneficial_owners[:3]:
                ...
                if owner and names and domains:
                    queries.append(f'"{owner}" ("{names[0]}" OR site:{domains[0]}) (CEO OR founder OR CTO OR CFO OR raises OR interview)')
    ...
    def collect_rss_feeds(self, site_url: Optional[str]) -> List[NewsItem]:
        ...
        if site_url.startswith("http"):
            ...
            candidates = [f"https://{site_url}/feed", f"https://{site_url}/rss", f"https://{site_url}"]
    ...
    @with_retry(
        max_attempts=2,
        retry_exceptions=(TimeoutException, ExternalServiceError)
    )
    ...
    def collect_company_news(
        ...
        if not self.config.cse_disable and self.config.google_api_key and self.config.google_cse_id:
            try:
                ...
                logger.warning("Google search collection failed", callsign=company.callsign, error=str(e))
...
class NewsService:
    ...
    def fetch_companies_data(self, subject_filter: Optional[str] = None) -> List[Company]:
        ...
        try:
            ...
            messages = self.gmail_client.search_messages(
                query=f'subject:"{subject}" has:attachment filename:csv',
                max_results=5
            )
            ...
            for msg_info in messages:
                try:
                    ...
                    logger.warning("Failed to process message", message_id=msg_info["id"], error=str(e))
                    continue
    ...
    def process_single_company(
        ...
        try:
            ...
            logger.error("Company intelligence processing failed", callsign=company.callsign, error=str(e))
    ...
    def run_intelligence_workflow(
        ...
        result = ProcessingResult(
            workflow_type=WorkflowType.NEWS,
        ...
        try:
            ...
            def process_company_wrapper(company: Company) -> CompanyIntelligence:
                return self.process_single_company(
                    company, 
                    enhanced_data.get(company.callsign.lower())
                )
            ...
            result.data = {
                ...
                "intelligence_items": sum(len(intel.news_items) for intel in intelligence_by_company.values()),
                "enhanced_companies": len([d for d in enhanced_data.values() if d.get("domain") or d.get("website")])
            }
    ...
    def _update_notion_intelligence(self, intelligence_by_company: Dict[str, CompanyIntelligence]) -> None:
    ...
    def _send_intelligence_digest(self, intelligence_by_company: Dict[str, CompanyIntelligence]) -> None:
