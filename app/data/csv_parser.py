...
from app.core.models import Company, AccountMovement, DigestStats
...
class CSVProcessor:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    company = Company(
                        ...
                        industry_tags=self.safe_string_conversion(row.get(cols.get("industry_tags"))),
                        ...
                        # Financial data
                        curr_balance=pd.to_numeric(row.get(cols.get("curr_balance")), errors="coerce"),
                        prev_balance=pd.to_numeric(row.get(cols.get("prev_balance")), errors="coerce"),
                        balance_delta=pd.to_numeric(row.get(cols.get("balance_delta")), errors="coerce"),
                        balance_pct_delta_pct=pd.to_numeric(row.get(cols.get("balance_pct_delta_pct")), errors="coerce")
                    )
                    
                    # Calculate any_change if not provided
                    if "any_change" not in cols:
                        company.any_change = (
                            company.is_new_account or company.is_removed_account or
                            company.dba_changed or company.website_changed or
                            company.owners_changed or company.balance_changed
                        )
                    else:
    ...
    def calculate_digest_data(self, companies: List[Company], top_n: int = 15) -> Dict[str, Any]:
        ...
        try:
            ...
            gainers = [
                c for c in companies_with_pct
                if c.balance_pct_delta_pct > 0
            ]
            ...
            # TODO: Parse product flips from JSON columns if available
            product_starts = []
            product_stops = []
    ...
    def extract_new_callsigns(self, companies: List[Company]) -> List[str]:
        ...
        """
        new_callsigns = [
            c.callsign for c in companies
            if c.is_new_account and c.callsign
        ]
    ...
    def validate_csv_data(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            numeric_columns = ["curr_balance", "prev_balance", "balance_delta", "balance_pct_delta_pct"]
            ...
            for col in boolean_columns:
                if col in cols:
                    ...
                    valid_boolean_values = {True, False, 0, 1, "true", "false", "True", "False", "0", "1"}
