...
from app.core.models import NotionPage, NotionUpdateOperation, Company
from app.utils.reliability import (
    with_circuit_breaker, with_retry, AdaptiveRateLimiter, default_rate_limiter
)
...
class EnhancedNotionClient:
    ...
    def _make_request(
        ...
        try:
            ...
            raise NotionError(error_msg, details={
                "status_code": e.response.status_code,
                "path": path
            })
    ...
    def _create_date_property(self, date_iso: Optional[str]) -> Dict[str, Any]:
        ...
        return {
            "date": {
                "start": date_iso or datetime.date.today().isoformat()
            }
        }
    ...
    def create_company_page(
        ...
        properties = {
            title_prop: self._create_title_segments(company.callsign)
        }
    ...
    def set_needs_dossier(self, page_id: str, needs: bool = True) -> None:
        ...
        if self.dry_run:
            logger.info(
                "DRY RUN: Would set needs dossier",
                page_id=page_id,
                needs=needs
            )
            return
    ...
    def get_company_domain_data(self, database_id: str, callsign: str) -> Dict[str, Optional[str]]:
        ...
        try:
            ...
            logger.error(
                "Failed to get company domain data",
                callsign=callsign,
                error=str(e)
            )
            return {"domain": None, "website": None}
