...
class NewsAnalyzer:
    ...
    def extract_themes(self, news_items: List[NewsItem]) -> List[str]:
        ...
        if type_counts[NewsType.FUNDRAISING] > 3:
            themes.append(f"Active funding period - {type_counts[NewsType.FUNDRAISING]} fundraising events")
        
        if type_counts[NewsType.PARTNERSHIP] > 2:
            themes.append(f"Partnership expansion - {type_counts[NewsType.PARTNERSHIP]} new partnerships")
        
        if type_counts[NewsType.LEADERSHIP] > 2:
            themes.append(f"Leadership changes - {type_counts[NewsType.LEADERSHIP]} executive updates")
        
        if type_counts[NewsType.PRODUCT_LAUNCH] > 2:
            themes.append(f"Product innovation - {type_counts[NewsType.PRODUCT_LAUNCH]} new launches")
    ...
    def identify_notable_items(self, news_items: List[NewsItem]) -> List[NewsItem]:
        ...
        funding_news = [item for item in news_items 
                       if item.news_type == NewsType.FUNDRAISING]
        notable.extend(funding_news)
        
        # Acquisition news (always notable)
        acquisition_news = [item for item in news_items 
                           if item.news_type == NewsType.ACQUISITION]
    ...
    def generate_weekly_digest(self, news_items: List[NewsItem], week_of: str) -> WeeklyNewsDigest:
        ...
        """
        logger.info("Generating weekly news digest", 
                   items=len(news_items), week=week_of)
    ...
    def _generate_summary_text(self, news_items: List[NewsItem]) -> str:
        ...
        if type_counts:
            ...
            summary_parts.append(f"with {top_type[0].value.replace('_', ' ')} being the most active category ({top_type[1]} items)")
        
        # Most active company
        if company_counts:
            top_company = company_counts.most_common(1)[0]
            if top_company[1] > 1:
                summary_parts.append(f"Most active company was {top_company[0]} with {top_company[1]} news items")
