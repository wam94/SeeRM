...
from app.intelligence.models import CompanyDeepDive, Report, ReportMetadata
...
class CompanyDeepDiveReport:
    ...
    def generate(self, callsign: str, include_email: bool = True) -> Report:
        ...
        logger.info("Generating company deep dive", callsign=callsign, report_id=report_id)
        ...
        try:
            ...
            logger.error("Failed to generate company deep dive", callsign=callsign, error=str(e))
            raise
    ...
    def _structure_report_content(self, deepdive: CompanyDeepDive) -> Dict[str, Any]:
        ...
        return {
            ...
            "movement": {
                "current_balance": (
                    deepdive.company.movement.current_balance if deepdive.company.movement else None
                ),
                "percentage_change": (
                    deepdive.company.movement.percentage_change
                    if deepdive.company.movement
                    else None
                ),
                "movement_type": (
                    deepdive.company.movement.movement_type.value
                    if deepdive.company.movement
                    else None
                ),
                "is_new_account": (
                    deepdive.company.movement.is_new_account if deepdive.company.movement else False
                ),
            },
            "news_summary": {
                "total_items": len(deepdive.news_timeline),
                "recent_items": len(
                    [n for n in deepdive.news_timeline if self._is_recent(n.published_at, 30)]
                ),
    ...
    def _render_html_report(self, deepdive: CompanyDeepDive) -> str:
        ...
        if deepdive.news_timeline:
            html_parts.extend(["<h2>Recent News (90 days)</h2>", "<ul>"])
        ...
        # Footer
        html_parts.extend(["<p><small>Generated by SeeRM Intelligence Reports</small></p>"])
    ...
    def _render_markdown_report(self, deepdive: CompanyDeepDive) -> str:
        ...
        # News Timeline
        if deepdive.news_timeline:
            md_parts.extend(["## Recent News (90 days)", ""])
            for news_item in deepdive.news_timeline[:5]:
                md_parts.append(f"### {news_item.title}")
                md_parts.append(
                    f"**Source:** {news_item.source} | **Date:** {news_item.published_at}"
                )
                if news_item.url:
    ...
    def _create_notion_report(self, report: Report, deepdive: CompanyDeepDive) -> Optional[str]:
        ...
        try:
            ...
            if page_id:
                logger.info(
                    "Company deep dive report created in Notion",
                    callsign=deepdive.company.profile.callsign,
                    page_id=page_id,
