...
from app.intelligence.models import MovementType, NewClientSummary, Report, ReportMetadata
...
class NewClientReport:
    ...
    def generate(
        self, new_callsigns: Optional[List[str]] = None, include_email: bool = True
    ) -> Optional[Report]:
        """
        ...
        logger.info("Generating new client report", week_of=week_of, report_id=report_id)
        ...
        try:
            ...
            for movement in all_movements:
                ...
                if client_balance > 0 and movement.current_balance > 0:
                    ratio = max(client_balance, movement.current_balance) / min(
                        client_balance, movement.current_balance
                    )
                    if ratio > 10:
                        continue
    ...
    def _create_consolidated_content(
        self, summaries: List[NewClientSummary], week_of: str
    ) -> Dict[str, Any]:
    ...
    def _render_markdown_report(self, summaries: List[NewClientSummary], week_of: str) -> str:
        ...
        for summary in summaries:
            ...
            if summary.similar_clients:
                md_parts.extend(["", f'**Similar Clients:** {", ".join(summary.similar_clients)}'])
    ...
    def _create_notion_report(
        self, report: Report, summaries: List[NewClientSummary]
    ) -> Optional[str]:
        ...
        try:
            ...
            if page_id:
                logger.info(
                    "New client report created in Notion",
                    client_count=len(summaries),
                    page_id=page_id,
            ...
            logger.error(
                "Failed to create Notion report", client_count=len(summaries), error=str(e)
            )
            return None
